...
function sweetTooth(totalNoOfSweets,sweetsConsumedByKaren,sweetsConsumedInMeters,metersToTravel){
    if(typeof totalNoOfSweets !== "number" ||
        ...
        typeof sweetsConsumedInMeters !== "number" ||
        typeof metersToTravel !== "number"){
            return "No sweets for Karen's friend";
        }
    let remaining = totalNoOfSweets -sweetsConsumedByKaren-(sweetsConsumedInMeters*metersToTravel);
    if(remaining<=0){
        return(0);
    }
    return remaining/2;
...
function consoleKaren(strategies){
    var strategiesLength = strategies.join(" ").length;
